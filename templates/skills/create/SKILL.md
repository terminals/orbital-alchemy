...
3. Fill in:
   - **Frontmatter**: id, title, category, created/updated dates, tags, **`status: exploring`**
   - **DASHBOARD**: Set status to `exploring`, add creation activity entry
...
```markdown
### Quick Status
> ⏳ **Status**: Exploring | **Phase**: 0 of N | **Spec Locked**: No
...
- [ ] Get spec approval via `/scope review`
```
...
Structure:
  PART 1: DASHBOARD  — Status: Exploring
...
3. Run `/scope review` to get agent team feedback
