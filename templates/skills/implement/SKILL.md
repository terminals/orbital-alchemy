...
# /implement-scope - Execute a Scope

## Modes

| Command | What Happens |
|---------|--------------|
| `/implement-scope 009` | Full workflow (review -> implement -> validate) |
| `/implement-scope 009 --review` | Review only (agents review -> update scope -> STOP) |
...
### 2. Team Review (if no review exists in scope doc)

Run the full `/scope-review` workflow (see `scope-review/SKILL.md`). This will:
- Launch agents, synthesize findings with dispositions (APPLY/CLARIFY/NOTE)
- Ask the user clarifying questions for ambiguous items
- Apply fixes directly to the SPECIFICATION
- Write the AGENT REVIEW section with Resolution traceability

If `--review` mode: STOP here after the review completes.
...
After Step 4, the scope remains in `scopes/implementing/` with `status: implementing`. Output:

```
╔═══════════════════════════════════════════════════════════════╗
║  Implementation complete. Scope ready for review.            ║
║                                                              ║
║  Next: /scope review-gate {NNN} (in a NEW session)           ║
║                                                              ║
