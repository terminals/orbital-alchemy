...
tokens: ~600
trigger: /pre-commit
purpose: Run the complete 8-step quality gate pipeline before committing
---
...
run_gate "no-stale-scopes" 12 13 'STALE=$(grep -l -E "Status:.*IMPLEMENTED|Status:.*Complete" scopes/*.md 2>/dev/null | wc -l | tr -d " "); [ "$STALE" -eq 0 ]'
...
| **Stale Scopes** | Run `/work save` to commit and transition reviewed scopes |
